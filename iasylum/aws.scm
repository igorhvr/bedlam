...
;; RedrivePolicy - The parameters for dead letter queue functionality of the source queue. For more information about RedrivePolicy and dead letter queues, see Using Amazon SQS Dead Letter Queues in the Amazon SQS Developer Guide.
   ...
   (define aws/make-s3-client
     (lambda* (credentials (region: region #f))
              (let ((result (j "new com.amazonaws.services.s3.AmazonS3Client(cred,new com.amazonaws.ClientConfiguration().withMaxConnections(2048).withConnectionTimeout(50000));" `((cred ,credentials)))))
   ...
   (define (aws/iam-client credentials) (j "new com.amazonaws.services.identitymanagement.AmazonIdentityManagementClient(credentials,new com.amazonaws.ClientConfiguration().withMaxConnections(2048).withConnectionTimeout(50000));" `((credentials ,credentials))))
   ...
   (define (aws/make-lambda-client credentials) (j "new com.amazonaws.services.lambda.AWSLambdaClient(credentials,new com.amazonaws.ClientConfiguration().withMaxConnections(2048).withConnectionTimeout(50000));" `((credentials ,credentials))))
