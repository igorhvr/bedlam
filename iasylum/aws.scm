...
;; RedrivePolicy - The parameters for dead letter queue functionality of the source queue. For more information about RedrivePolicy and dead letter queues, see Using Amazon SQS Dead Letter Queues in the Amazon SQS Developer Guide.
   ...
   (define (aws/make-dynamodb-client credentials) (j "new com.amazonaws.services.dynamodbv2.AmazonDynamoDBClient(credentials,new com.amazonaws.ClientConfiguration().withMaxConnections(250).withConnectionTimeout(50000));" `((credentials ,credentials))))
   ...
   (define aws/make-s3-client
     (lambda* (credentials (region: region #f))
              (let ((result (j "new com.amazonaws.services.s3.AmazonS3Client(cred,new com.amazonaws.ClientConfiguration().withMaxConnections(250).withConnectionTimeout(50000));" `((cred ,credentials)))))
   ...
   (define aws/s3-put-binary
     (lambda* (s3-client bucket object byte-array-input-stream
                 ...
                 (auto-retry: auto-retry (j "new com.google.api.client.util.ExponentialBackOff();")))
           (let try-again ()
             (with-failure-continuation
              (lambda (error error-continuation)
                (if (not auto-retry)
                    (log-error "Error when trying to send something to S3. I'll give up, there is no auto retry.")
                    (let ((ms-to-wait (->scm-object (j "backoff.getCurrentIntervalMillis();" `((backoff ,auto-retry)))))
                      ...
                      (log-error (format "Error when trying to send something to S3, I'll try again after ~a ms. Elapsed: ~a ms, Max: ~a ms."
                                         ms-to-wait ms-elapsed ms-max) error)
                      (sleep ms-to-wait)
   ...
   (define (aws/iam-client credentials) (j "new com.amazonaws.services.identitymanagement.AmazonIdentityManagementClient(credentials,new com.amazonaws.ClientConfiguration().withMaxConnections(250).withConnectionTimeout(50000));" `((credentials ,credentials))))
   ...
   (define (aws/make-lambda-client credentials) (j "new com.amazonaws.services.lambda.AWSLambdaClient(credentials,new com.amazonaws.ClientConfiguration().withMaxConnections(250).withConnectionTimeout(50000));" `((credentials ,credentials))))
