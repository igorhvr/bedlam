;;;; fmt-unicode-chicken.scm -- Unicode character width and ANSI escape support
;;
;; Copyright (c) 2006-2009 Alex Shinn.  All rights reserved.
;; BSD-style license: http://synthcode.com/license.txt

(require-library srfi-4 fmt utf8-lolevel)

(module
 fmt-unicode
 (unicode-char-width unicode-string-width fmt-unicode)

(import scheme chicken srfi-4 fmt utf8-lolevel)
      ...
      (if (>= i end)
          ...
          (let ((c (string-ref str i)))
            (cond
              ...
              ;; unicode characters
              ((>= (char->integer c) #x80)
               (cond-expand ; XXXX non-top-level!
                (chicken
                 (let ((c-len (utf8-start-byte->length (char->integer c))))
                   (lp1 (+ i c-len)
                        (+ width (unicode-char-width (sp-ref str i))))))
                (else
