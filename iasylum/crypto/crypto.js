...
var iasylum_crypto = {
    'generate_sjcl_el_gammal_ecc_c192_keypair' : function() {
	var keypair=sjcl.ecc.elGamal.generateKeys(sjcl.ecc.curves['c192'],0);
        var resultpub={};
        var resultsec={};
        resultpub['pub']=JSON.parse(JSON.stringify(keypair.pub.get()));
        resultsec['sec']=JSON.parse(JSON.stringify(keypair.sec.get()));
        var finalresultpub={'type' : 'sjcl_el_gammal_ecc_c192_key', 'body' : resultpub};
        var finalresultsec={'type' : 'sjcl_el_gammal_ecc_c192_key', 'body' : resultsec};
	...
	var key = JSON.parse(stringKey);
        ...
        if(key['type'] == 'sjcl_el_gammal_ecc_c192_key') {
