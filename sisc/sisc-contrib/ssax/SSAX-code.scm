...
(define (SSAX:read-pi-body-as-string port) (SSAX:skip-S port) (apply string-append (let loop () (let ((pi-fragment (next-token '() '(#\?) "reading PI content" port))) (if (eq? #\> (peek-next-char port)) (begin (read-char port) (cons pi-fragment '())) (cons* pi-fragment "?" (loop)))))))
...
(define SSAX:read-CDATA-body (let ((nl-str (string #\newline))) (lambda (port str-handler seed) (let loop ((seed seed)) (let ((fragment (next-token '() (list #\newline #\] #\&) "reading CDATA" port))) (case (read-char port) ((#\newline) (loop (str-handler fragment nl-str seed))) ((#\]) (if (not (eqv? (peek-char port) #\])) (loop (str-handler fragment "]" seed)) (let check-after-second-braket ((seed (if (string-null? fragment) seed (str-handler fragment "" seed)))) (case (peek-next-char port) ((#\>) (read-char port) seed) ((#\]) (check-after-second-braket (str-handler "]" "" seed))) (else (loop (str-handler "]]" "" seed))))))) ((#\&) (let ((ent-ref (next-token-of (lambda (c) (and (not (eof-object? c)) (char-alphabetic? c) c)) port))) (cond ((and (string=? "gt" ent-ref) (eq? (peek-char port) #\;)) (read-char port) (loop (str-handler fragment ">" seed))) (else (loop (str-handler ent-ref "" (str-handler fragment "&" seed))))))) (else (parser-error port "can't happen"))))))))

(define (SSAX:read-char-ref port) (let* ((base (cond ((eq? (peek-char port) #\x) (read-char port) 16) (else 10))) (name (next-token '() '(#\;) "XML [66]" port)) (char-code (string->number name base))) (read-char port) (if (integer? char-code) (ascii->char char-code) (parser-error port "[wf-Legalchar] broken for '" name "'"))))
