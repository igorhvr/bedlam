...
sjcl.cipher.aes.prototype = {
    encrypt: function (a) {
        ...
        for (f = g = 0; !c[f]; f ^= l || 1, g = k[g] || 1) {
            ...
            for (e = 0; 4 > e; e++)a[e][f] = n = n << 24 ^ n >>> 8, b[e][m] = q = q << 24 ^ q >>> 8
        }
        for (e =
...
function D(a, b, c) {
    ...
    for (m = 0; m < n; m++)h = a[e >>> 24] ^ s[f >> 16 & 255] ^ E[g >> 8 & 255] ^ G[b & 255] ^ d[q], k = a[f >>> 24] ^ s[g >> 16 & 255] ^ E[b >> 8 & 255] ^ G[e & 255] ^ d[q + 1], l = a[g >>> 24] ^ s[b >> 16 & 255] ^ E[e >> 8 & 255] ^ G[f & 255] ^ d[q + 2], b = a[b >>> 24] ^ s[e >> 16 & 255] ^ E[f >> 8 & 255] ^ G[g & 255] ^ d[q + 3], q += 4, e = h, f = k, g = l;
    for (m = 0; 4 >
    m; m++)t[c ? 3 & -m : m] = F[e >>> 24] << 24 ^ F[f >> 16 & 255] << 16 ^ F[g >> 8 & 255] << 8 ^ F[b & 255] ^ d[q++], h = e, e = f, f = g, g = b, b = h;
    return t
}
sjcl.bitArray = {
    bitSlice: function (a, b, c) {
        ...
        if (sjcl.bitArray.bitLength(a) !== sjcl.bitArray.bitLength(b))return C;
        var c = 0, d;
...
sjcl.codec.base64 = {
    W: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", fromBits: function (a, b, c) {
        var d = "", e = 0, f = sjcl.codec.base64.W, g = 0, h = sjcl.bitArray.bitLength(a);
        c && (f = f.substr(0, 62) + "-_");
        ...
        a = a.replace(/\s|=/g, "");
        var c = [], d, e = 0, f = sjcl.codec.base64.W, g = 0, h;
        b && (f = f.substr(0, 62) + "-_");
...
sjcl.hash.sha256 = function (a) {
    this.d[0] || this.F();
    a ? (this.f = a.f.slice(0), this.e = a.e.slice(0), this.c = a.c) : this.reset()
};
...
sjcl.hash.sha256.prototype = {
    blockSize: 512, reset: function () {
        ...
        for (b = 512 + b & -512; b <= a; b += 512)this.j(c.splice(0, 16));
        return this
    }, finalize: function () {
        var a, b = this.e, c = this.f, b = sjcl.bitArray.concat(b, [sjcl.bitArray.partial(1, 1)]);
        for (a = b.length + 2; a & 15; a++)b.push(0);
        b.push(Math.floor(this.c /
            4294967296));
        for (b.push(this.c | 0); b.length;)this.j(b.splice(0, 16));
        this.reset();
        return c
    }, l: [], d: [], F: function () {
        function a(a) {
        ...
        var b, c, d = a.slice(0), e = this.f, f = this.d, g = e[0], h = e[1], k = e[2], l = e[3], n = e[4], m = e[5], q = e[6], t = e[7];
...
sjcl.hash.sha512.prototype = {
    ...
    update: function (a) {
        ...
        b = this.c;
        a = this.c = b + sjcl.bitArray.bitLength(a);
        for (b = 1024 + b & -1024; b <= a; b += 1024)this.j(c.splice(0, 32));
        return this
    },
    finalize: function () {
        var a, b = this.e, c = this.f, b = sjcl.bitArray.concat(b, [sjcl.bitArray.partial(1, 1)]);
        ...
        for (b.push(this.c | 0); b.length;)this.j(b.splice(0, 32));
        this.reset();
        return c
    },
    l: [],
    qa: [12372232, 13281083, 9762859, 1914609, 15106769, 4090911, 4308331, 8266105],
    d: [],
    ra: [2666018, 15689165, 5061423, 9034684, 4764984, 380953, 1658779, 7176472, 197186, 7368638, 14987916, 16757986, 8096111, 1480369, 13046325, 6891156, 15813330, 5187043, 9229749, 11312229, 2818677, 10937475, 4324308, 1135541, 6741931, 11809296, 16458047, 15666916, 11046850, 698149, 229999, 945776, 13774844, 2541862, 12856045, 9810911, 11494366,
    ...
    F: function () {
        ...
        a:for (; 80 >
            ...
            8 > c && (this.l[2 * c] = a(Math.pow(d, 0.5)), this.l[2 * c + 1] = b(Math.pow(d, 0.5)) << 24 | this.qa[c]);
            this.d[2 * c] = a(Math.pow(d, 1 / 3));
            this.d[2 * c + 1] = b(Math.pow(d, 1 / 3)) << 24 | this.ra[c];
            c++
        }
    },
    j: function (a) {
        var b, c, d = a.slice(0), e = this.f, f = this.d, g = e[0], h = e[1], k = e[2], l = e[3], n = e[4], m = e[5], q = e[6], t = e[7], s = e[8], E = e[9], G = e[10], F = e[11], aa = e[12], Q = e[13], ba = e[14], R = e[15], y = g, v = h, J = k, H = l, K = n, I = m, V = q, L = t, z = s, w = E, S = G, M = F, T = aa, N = Q, W = ba, O = R;
        for (a = 0; 80 > a; a++) {
            if (16 > a)b = d[2 * a], c = d[2 * a +
            1]; else {
                c = d[2 * (a - 15)];
                var u = d[2 * (a - 15) + 1];
                ...
                c = d[2 * (a - 2)];
                var B = d[2 * (a - 2) + 1], u = (B << 13 | c >>> 19) ^ (c << 3 | B >>> 29) ^ c >>> 6, B = (c << 13 | B >>> 19) ^ (B << 3 | c >>> 29) ^ (c << 26 | B >>> 6), X = d[2 * (a - 7)], Y = d[2 * (a - 16)], P = d[2 * (a - 16) + 1];
                c = A + d[2 * (a - 7) + 1];
                b = b + X + (c >>> 0 < A >>> 0 ? 1 : 0);
                c += B;
                b += u + (c >>> 0 < B >>> 0 ? 1 : 0);
            ...
            d[2 * a] = b |= 0;
            d[2 * a + 1] = c |= 0;
            var X = z & S ^ ~z & T, ca = w & M ^ ~w & N, B = y & J ^ y & K ^ J & K, ga = v & H ^ v & I ^ H & I, Y = (v << 4 | y >>> 28) ^ (y << 30 | v >>> 2) ^ (y << 25 | v >>>
                7), P = (y << 4 | v >>> 28) ^ (v << 30 | y >>> 2) ^ (v << 25 | y >>> 7), ha = f[2 * a], da = f[2 * a + 1], u = O + ((z << 18 | w >>> 14) ^ (z << 14 | w >>> 18) ^ (w << 23 | z >>> 9)), A = W + ((w << 18 | z >>> 14) ^ (w << 14 | z >>> 18) ^ (z << 23 | w >>> 9)) + (u >>> 0 < O >>> 0 ? 1 : 0), u = u + ca, A = A + (X + (u >>> 0 < ca >>> 0 ? 1 : 0)), u = u + da, A = A + (ha + (u >>> 0 < da >>> 0 ? 1 : 0)), u = u + c, A = A + (b + (u >>> 0 < c >>> 0 ? 1 : 0));
            c = P + ga;
            b = Y + B + (c >>> 0 < P >>> 0 ? 1 : 0);
            W = T;
            O = N;
        ...
        t = e[7] = t + L | 0;
        e[6] = q + V + (t >>> 0 < L >>> 0 ? 1 : 0) | 0;
        E = e[9] = E + w | 0;
        e[8] = s + z + (E >>> 0 < w >>> 0 ? 1 : 0) | 0;
        F = e[11] = F + M | 0;
        e[10] = G + S + (F >>> 0 < M >>> 0 ? 1 : 0) | 0;
        Q = e[13] = Q + N | 0;
        e[12] = aa + T + (Q >>> 0 < N >>> 0 ? 1 : 0) | 0;
        R = e[15] = R + O | 0;
        e[14] = ba + W + (R >>> 0 < O >>> 0 ? 1 : 0) | 0
    }
};
...
sjcl.hash.sha1.prototype = {
    ...
    update: function (a) {
        ...
        b = this.c;
        a = this.c = b + sjcl.bitArray.bitLength(a);
        for (b = this.blockSize + b & -this.blockSize; b <= a; b += this.blockSize)this.j(c.splice(0, 16));
        return this
    },
    finalize: function () {
        var a, b = this.e, c = this.f, b = sjcl.bitArray.concat(b, [sjcl.bitArray.partial(1, 1)]);
    ...
    j: function (a) {
        var b, c, d, e, f, g, h = a.slice(0), k = this.f;
        c = k[0];
        d = k[1];
        e = k[2];
        f = k[3];
        g = k[4];
        for (a = 0; 79 >= a; a++)16 <= a && (h[a] = (h[a - 3] ^ h[a - 8] ^ h[a - 14] ^ h[a - 16]) << 1 | (h[a - 3] ^ h[a - 8] ^ h[a - 14] ^ h[a - 16]) >>> 31), b = 19 >= a ? d & e | ~d & f : 39 >= a ? d ^ e ^ f : 59 >= a ? d & e | d & f | e & f : 79 >= a ? d ^ e ^ f : r, b = (c << 5 | c >>> 27) + b + g + h[a] + this.d[Math.floor(a /
...
sjcl.beware["CBC mode is dangerous because it doesn't protect message integrity."] = function () {
    sjcl.mode.cbc = {
        name: "cbc", encrypt: function (a, b, c, d) {
            ...
            var e = sjcl.bitArray, f = e.n, g = e.bitLength(b), h = 0, k = [];
...
sjcl.mode.ocb2 = {
    name: "ocb2", encrypt: function (a, b, c, d, e, f) {
        ...
        var g, h = sjcl.mode.ocb2.U, k = sjcl.bitArray, l = k.n, n = [0, 0, 0, 0];
        c = h(a.encrypt(c));
        var m, q = [];
        d = d || [];
        e = e || 64;
        for (g = 0; g + 4 < b.length; g += 4)m = b.slice(g, g + 4), n = l(n, m), q = q.concat(l(c, a.encrypt(l(c, m)))), c = h(c);
        m = b.slice(g);
        b = k.bitLength(m);
        ...
        128 !== sjcl.bitArray.bitLength(c) && p(new sjcl.exception.invalid("ocb iv must be 128 bits"));
        e = e || 64;
        var g = sjcl.mode.ocb2.U, h = sjcl.bitArray, k = h.n, l = [0, 0, 0, 0], n = g(a.encrypt(c)), m, q, t = sjcl.bitArray.bitLength(b) - e, s = [];
        d = d || [];
        for (c = 0; c + 4 < t / 32; c += 4)m = k(n, a.decrypt(k(n, b.slice(c, c + 4)))), l = k(l, m), s = s.concat(m), n = g(n);
        q = t - 32 * c;
        ...
        var c, d = sjcl.mode.ocb2.U, e = sjcl.bitArray, f = e.n, g = [0, 0, 0, 0], h = a.encrypt([0, 0, 0, 0]), h = f(h, d(d(h)));
...
sjcl.mode.gcm = {
    name: "gcm", encrypt: function (a, b, c, d, e) {
        ...
        b = sjcl.bitArray;
        d = d || [];
        a = sjcl.mode.gcm.D(x, a, f, d, c, e || 128);
        ...
        var d, e = c.length;
        b = b.slice(0);
        for (d = 0; d < e; d += 4)b[0] ^= 0xffffffff & c[d], b[1] ^= 0xffffffff & c[d + 1], b[2] ^= 0xffffffff & c[d + 2], b[3] ^= 0xffffffff & c[d + 3], b = sjcl.mode.gcm.oa(b, a);
        return b
    }, D: function (a, b, c, d, e, f) {
...
sjcl.misc.hmac.prototype.digest = function () {
    var a = this.T.finalize(), a = (new this.$(this.C[1])).update(a).finalize();
    this.reset();
    return a
};
...
sjcl.prng = function (a) {
    ...
    this.R = 0;
    this.X = {};
    this.da = this.m = this.w = this.la = 0;
    this.d = [0, 0, 0, 0, 0, 0, 0, 0];
    this.p = [0, 0, 0, 0];
    this.P = r;
    this.Q = a;
    this.H = C;
    this.O = {progress: {}, seeded: {}};
    this.B = this.ka = 0;
    this.L = 1;
    this.M = 2;
    this.ia = 0x10000;
    this.V = [0, 48, 64, 96, 128, 192, 0x100, 384, 512, 768, 1024];
    this.ja = 3E4;
    this.ha = 80
};
sjcl.prng.prototype = {
    randomWords: function (a, b) {
        ...
        if (d & this.M) {
            ...
            e = [];
            var f = 0, g;
            this.da = e[0] = 171503 + this.ja;
            for (g = 0; g < this.i.length && !(e = e.concat(this.i[g].finalize()), f += this.u[g], this.u[g] = 0, !d && this.S & 1 << g); g++);
            this.S >= 1 << this.i.length && (this.i.push(new sjcl.hash.sha256), this.u.push(0));
            this.m -= f;
            f > this.w && (this.w = f);
            this.S++;
            this.d = sjcl.hash.sha256.hash(this.d.concat(e));
            this.P = new sjcl.cipher.aes(this.d);
            for (d = 0; 4 > d && !(this.p[d] = this.p[d] + 1 | 0, this.p[d]); d++);
        }
        for (d = 0; d < a; d += 4)0 === (d + 1) % this.ia && ea(this), e = U(this), c.push(e[0], e[1], e[2], e[3]);
        ea(this);
        return c.slice(0, a)
        ...
        c = c || "user";
        var d, e, f = this.K[c], g = this.isReady(), h = 0;
        d = this.X[c];
        d === r && (d = this.X[c] = this.la++);
        f === r && (f = this.K[c] =
            0);
        this.K[c] = (this.K[c] + 1) % this.i.length;
        switch (typeof a) {
            case "number":
                b === r && (b = 1);
                this.i[f].update([d, this.R++, 1, b, 71503, 1, a | 0]);
                break;
            case "object":
                ...
                if ("[object Uint32Array]" === c) {
                    ...
                    "[object Array]" !== c && (h = 1);
                    for (c = 0; c < a.length && !h; c++)"number" !== typeof a[c] && (h = 1)
                }
                if (!h) {
                    if (b === r)for (c = b = 0; c < a.length; c++)for (e = a[c]; 0 < e;)b++, e >>>= 1;
                    this.i[f].update([d, this.R++, 2, b, 71503, a.length].concat(a))
                }
                break;
            case "string":
                b ===
                r && (b = a.length);
                this.i[f].update([d, this.R++, 3, b, 71503, a.length]);
                this.i[f].update(a);
                break;
            default:
                h = 1
        }
        h && p(new sjcl.exception.bug("random: addEntropy only supports number, array of numbers or string"));
        this.u[f] += b;
        this.m += b;
        g === this.B && (this.isReady() !== this.B && fa("seeded", Math.max(this.w, this.m)), fa("progress", this.getProgress()))
    }, isReady: function (a) {
        return true;
        a = this.V[a !== r ? a : this.Q];
        return this.w && this.w >= a ? this.u[0] > this.ha && 0 > this.da ? this.M | this.L : this.L : this.m >= a ? this.M | this.B : this.B
    }, getProgress: function (a) {
        a =
            this.V[a ? a : this.Q];
        return this.w >= a ? 1 : this.m > a ? 1 : this.m / a
    }, startCollectors: function () {
        this.H || (this.H = x)
    }, stopCollectors: function () {
        this.H && (window.removeEventListener ? (window.removeEventListener("load", this.s.loadTimeCollector, C), window.removeEventListener("mousemove", this.s.mouseCollector, C), window.removeEventListener("keypress", this.s.keyboardCollector, C), window.removeEventListener("devicemotion", this.s.accelerometerCollector, C)) : document.detachEvent && (document.detachEvent("onload", this.s.loadTimeCollector),
            document.detachEvent("onmousemove", this.s.mouseCollector), document.detachEvent("keypress", this.s.keyboardCollector)), this.H = C)
    }, addEventListener: function (a, b) {
        this.O[a][this.ka++] = b
    }, removeEventListener: function (a, b) {
        var c, d, e = this.O[a], f = [];
        ...
        sjcl.random.addEntropy([a.x || a.clientX || a.offsetX || 0, a.y || a.clientY ||
        a.offsetY || 0], 2, "mouse");
        Z(0)
    }, xa: function () {
        Z(2)
...
function fa(a, b) {
    var c, d = sjcl.random.O[a], e = [];
    for (c in d)d.hasOwnProperty(c) && e.push(d[c]);
    for (c = 0; c < e.length; c++)e[c](b)
}
function Z(a) {
    window && window.performance && "function" === typeof window.performance.now ? sjcl.random.addEntropy(window.performance.now(), a, "loadtime") : sjcl.random.addEntropy(7117, a, "loadtime")
}
function ea(a) {
    a.d = U(a).concat(U(a));
    a.P = new sjcl.cipher.aes(a.d)
}
function U(a) {
    for (var b = 0; 4 > b && !(a.p[b] = a.p[b] + 1 | 0, a.p[b]); b++);
    return a.P.encrypt(a.p)
}
sjcl.random = new sjcl.prng(0);
sjcl.json = {
    defaults: {v: 1, iter: 1E3, ks: 128, ts: 64, mode: "ccm", adata: "", cipher: "aes"}, na: function (a, b, c, d) {
        c = c || {};
        d = d || {};
        var e = sjcl.json, f = e.o({iv: sjcl.random.randomWords(4, 0)}, e.defaults), g;
        e.o(f, c);
        c = f.adata;
        ...
        return f
                          ...
                          b, c, d) {
        var e = sjcl.json, f = e.na.apply(e, arguments);
        return e.encode(f)
    }, ma: function (a, b, c, d) {
        c = c || {};
        d = d || {};
        var e = sjcl.json;
        b = e.o(e.o(e.o({}, e.defaults), b), c, x);
        var f;
        c = b.adata;
        "string" === typeof b.salt && (b.salt = sjcl.codec.base64.toBits(b.salt));
        "string" === typeof b.iv && (b.iv = sjcl.codec.base64.toBits(b.iv));
        (!sjcl.mode[b.mode] || !sjcl.cipher[b.cipher] || "string" === typeof a && 100 >= b.iter || 64 !== b.ts && 96 !== b.ts && 128 !== b.ts || 128 !== b.ks && 192 !== b.ks && 0x100 !== b.ks || !b.iv || 2 > b.iv.length || 4 < b.iv.length) && p(new sjcl.exception.invalid("json decrypt: invalid parameters"));
        ...
        var e = sjcl.json;
        return e.ma(a, e.decode(b), c, d)
    }, encode: function (a) {
        var b,
            c = "{", d = "";
        for (b in a)if (a.hasOwnProperty(b))switch (b.match(/^[a-z0-9]+$/i) || p(new sjcl.exception.invalid("json encode: invalid property name")), c += d + '"' + b + '":', d = ",", typeof a[b]) {
            case "number":
            case "boolean":
                c += a[b];
                break;
        ...
        a = a.replace(/^\{|\}$/g, "").split(/,/);
        var b = {}, c, d;
        for (c = 0; c < a.length; c++)(d = a[c].match(/^(?:(["']?)([a-z][a-z0-9]*)\1):(?:(\d+)|"([a-z0-9+\/%*_.@=\-]*)")$/i)) || p(new sjcl.exception.invalid("json decode: this isn't json!")), b[d[2]] = d[3] ? parseInt(d[3], 10) : d[2].match(/^(ct|salt|iv)$/) ? sjcl.codec.base64.toBits(d[4]) : unescape(d[4]);
        return b
    }, o: function (a, b, c) {
        a === r && (a = {});
        if (b === r)return a;
        for (var d in b)b.hasOwnProperty(d) && (c && (a[d] !== r && a[d] !== b[d]) && p(new sjcl.exception.invalid("required parameter overridden")),
            a[d] = b[d]);
        return a
    }, za: function (a, b) {
        var c = {}, d;
...
sjcl.bn.prototype = {
    radix: 24, maxMul: 8, h: sjcl.bn, copy: function () {
        ...
        "number" === typeof a && (a = new this.h(a));
        var b, c = this.limbs, d = a.limbs, e = c.length, f = d.length, g = new this.h, h = g.limbs, k, l = this.maxMul;
        ...
        var b, c, d = new this.h(1), e = this;
        for (b = 0; b < a.length; b++)for (c = 0; c < this.radix; c++)a[b] & 1 << c && (d = d.mul(e)), e = e.square();
        return d
    }, mulmod: function (a, b) {
        ...
        for (b = 0; b < f || 0 !== a && -1 !== a; b++)a = (e[b] || 0) + a, d = e[b] = a & g, a = (a - d) * c;
...
var $ = sjcl.bn.pseudoMersennePrime;
sjcl.bn.prime = {
    p127: $(127, [[0, -1]]),
    p25519: $(255, [[0, -19]]),
    p192k: $(192, [[32, -1], [12, -1], [8, -1], [7, -1], [6, -1], [3, -1], [0, -1]]),
    p224k: $(224, [[32, -1], [12, -1], [11, -1], [9, -1], [7, -1], [4, -1], [1, -1], [0, -1]]),
    p256k: $(0x100, [[32, -1], [9, -1], [8, -1], [7, -1], [6, -1], [4, -1], [0, -1]]),
    p192: $(192, [[0, -1], [64, -1]]),
    p224: $(224, [[0, 1], [96, -1]]),
    p256: $(0x100, [[0, -1], [96, 1], [192, 1], [224, -1]]),
    p384: $(384, [[0, -1], [32, 1], [96, -1], [128, -1]]),
    p521: $(521, [[0, -1]])
};
...
sjcl.ecc.point = function (a, b, c) {
    b === r ? this.isIdentity = x : (this.x = b, this.y = c, this.isIdentity = C);
    this.curve = a
};
...
sjcl.ecc.pointJac = function (a, b, c, d) {
    b === r ? this.isIdentity = x : (this.x = b, this.y = c, this.z = d, this.isIdentity = C);
    this.curve = a
};
sjcl.ecc.pointJac.prototype = {
    add: function (a) {
        ...
        for (c = a.length - 1; 0 <= c; c--)for (d = sjcl.bn.prototype.radix - 4; 0 <= d; d -= 4)e = e.doubl().doubl().doubl().doubl().add(f[a[c] >>
        d & 15]);
        return e
        ...
        for (d = Math.max(a.length, c.length) - 1; 0 <= d; d--) {
            h = a[d] | 0;
            k = c[d] | 0;
            for (e = sjcl.bn.prototype.radix - 4; 0 <= e; e -= 4)f = f.doubl().doubl().doubl().doubl().add(b[h >> e & 15]).add(g[k >> e & 15])
        }
        return f
    }, isValid: function () {
...
sjcl.ecc.basicKey = {
    publicKey: function (a, b) {
        this.k = a;
        this.q = a.r.bitLength();
        this.J = b instanceof Array ? a.fromBits(b) : b;
        this.get = function () {
            var a = this.J.toBits(), b = sjcl.bitArray.bitLength(a), e = sjcl.bitArray.bitSlice(a, 0, b / 2), a = sjcl.bitArray.bitSlice(a, b / 2);
            return {x: e, y: a}
        }
...
sjcl.ecc.elGamal.publicKey.prototype = {
    kem: function (a) {
        a = sjcl.bn.random(this.k.r, a);
        var b = this.k.G.mult(a).toBits();
        return {key: sjcl.hash.sha256.hash(this.J.mult(a).toBits()), tag: b}
    }
};
sjcl.ecc.elGamal.secretKey.prototype = {
    unkem: function (a) {
        return sjcl.hash.sha256.hash(this.k.fromBits(a).mult(this.I).toBits())
    }, dh: function (a) {
        return sjcl.hash.sha256.hash(a.J.mult(this.I).toBits())
    }
};
...
sjcl.ecc.ecdsa.publicKey.prototype = {
    verify: function (a, b, c) {
        sjcl.bitArray.bitLength(a) > this.q && (a = sjcl.bitArray.clamp(a, this.q));
        var d = sjcl.bitArray, e = this.k.r, f = this.q, g = sjcl.bn.fromBits(d.bitSlice(b, 0, f)), d = sjcl.bn.fromBits(d.bitSlice(b, f, 2 * f)), h = c ? d : d.inverseMod(e), f = sjcl.bn.fromBits(a).mul(h).mod(e), h = g.mul(h).mod(e), f = this.k.G.mult2(f, h, this.J).x;
        if (g.equals(0) || d.equals(0) || g.greaterEquals(e) || d.greaterEquals(e) || !f.equals(g)) {
            if (c === r)return this.verify(a, b, x);
...
sjcl.ecc.ecdsa.secretKey.prototype = {
    sign: function (a, b, c, d) {
        sjcl.bitArray.bitLength(a) > this.q && (a = sjcl.bitArray.clamp(a, this.q));
        var e = this.k.r, f = e.bitLength();
        d = d || sjcl.bn.random(e.sub(1), b).add(1);
        b = this.k.G.mult(d).x.mod(e);
        a = sjcl.bn.fromBits(a).add(b.mul(this.I));
...
sjcl.keyexchange.srp = {
    makeVerifier: function (a, b, c, d) {
        ...
        sjcl.keyexchange.srp.Z || sjcl.keyexchange.srp.pa();
        return sjcl.keyexchange.srp.ba[a]
    }, Z: C, pa: function () {
        var a, b;
        for (a = 0; a < sjcl.keyexchange.srp.aa.length; a++)b = sjcl.keyexchange.srp.aa[a].toString(),
            b = sjcl.keyexchange.srp.ba[b], b.N = new sjcl.bn(b.N), b.g = new sjcl.bn(b.g);
        sjcl.keyexchange.srp.Z = x
    }, aa: [1024, 1536, 2048], ba: {
        1024: {
